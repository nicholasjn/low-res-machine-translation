...
def split_data(src_lines: list[str], tgt_lines: list[str]):
    ...
    if len(src_lines) < needed:
        raise ValueError(
            f"Cleaned corpus has {len(src_lines)} sentence pairs, but dev+test requires {needed}."
        )
...
def parse_args():
    parser = argparse.ArgumentParser(description="Create deterministic cleaned KDE4 de-hsb splits.")
    parser.add_argument("--src_path", default="../data/cleaned/KDE4.cleaned.de")
    parser.add_argument("--ref_path", default="../data/cleaned/KDE4.cleaned.hsb")
    parser.add_argument("--output_dir", default="../data/splits_cleaned")
...
def main():
    ...
    print(f"total cleaned pairs: {len(src_lines)}")
    ...
    print("Saved cleaned splits to:", output_dir)
